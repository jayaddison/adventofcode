...
complete_boards = set(boards)
for number in draw_numbers:
    for board_idx, board in enumerate(boards):
        board.draw(number)
        if board.has_won() and board in complete_boards:
            complete_boards.remove(board)
        if len(complete_boards) == 0:
            print(f"Board {board_idx} is last to win with score: {number * board.product()}")
            break
    if len(complete_boards) == 0:
        break
